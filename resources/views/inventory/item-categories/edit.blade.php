...
@section('title', 'تعديل فئة منتجات - '.config('app.name'))
...
<div dir="rtl" class="mx-auto w-full max-w-4xl space-y-6">
    <header class="flex flex-wrap items-center justify-between gap-4">
        <h1 class="inline-flex items-center gap-2 text-2xl font-bold text-gray-900">
            <span>تعديل: {{ $itemCategory->name }}</span>
    ...
    <form method="POST" action="{{ route('inventory.item-categories.update', $itemCategory) }}" class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $itemCategory->name) }}" required maxlength="150" class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="text" name="name_ar" value="{{ old('name_ar', $itemCategory->name_ar) }}" maxlength="150" class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <select name="is_active" class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
                    <option value="1" @selected(old('is_active', $itemCategory->is_active ? '1' : '0') == '1' || old('is_active', $itemCategory->is_active ? '1' : '0') === 1)>نشط</option>
                    <option value="0" @selected(old('is_active', $itemCategory->is_active ? '1' : '0') == '0' || old('is_active', $itemCategory->is_active ? '1' : '0') === 0)>موقوف</option>
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <x-searchable-select
                    ...
                    :options="$invOpts"
                    :value="old('inventory_account_id', $itemCategory->inventory_account_id)"
                    :required="true"
            ...
            <div class="md:col-span-2">
                ...
                <x-searchable-select
                    ...
                    :options="$revOpts"
                    :value="old('sales_income_account_id', $itemCategory->sales_income_account_id)"
                    :required="true"
            ...
            <div class="md:col-span-2">
                ...
                <x-searchable-select
                    ...
                    id="cogs_account_id"
                    :options="$cogsOpts"
                    :value="old('cogs_account_id', $itemCategory->cogs_account_id)"
                    :required="true"
