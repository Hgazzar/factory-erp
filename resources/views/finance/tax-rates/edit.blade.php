...
@section('title', 'تعديل ضريبة - '.config('app.name'))
...
<div dir="rtl" class="mx-auto w-full max-w-3xl space-y-6">
    <header class="flex flex-wrap items-center justify-between gap-4">
        <h1 class="inline-flex items-center gap-2 text-2xl font-bold text-gray-900">
            <span>تعديل ضريبة: {{ $taxRate->code }}</span>
    ...
    <form method="POST" action="{{ route('finance.tax-rates.update', $taxRate) }}" class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $taxRate->code) }}" required maxlength="32" class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="number" name="rate_percent" value="{{ old('rate_percent', $taxRate->rate_percent) }}" required min="0" max="100" step="0.0001" class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm tabular-nums focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="name_ar" value="{{ old('name_ar', $taxRate->name_ar) }}" required class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="name_en" value="{{ old('name_en', $taxRate->name_en) }}" class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="md:col-span-2">
                ...
                <x-searchable-select
                    ...
                    :value="old('ledger_account_id', $taxRate->ledger_account_id)"
                    :required="true"
