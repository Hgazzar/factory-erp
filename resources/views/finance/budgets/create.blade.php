...
@section('title', 'ميزانية جديدة - MIRADA ERP')
...
@section('breadcrumb')
    ...
    <span class="mx-1 text-gray-400">›</span>
    <span class="text-blue-900 font-semibold">ميزانية جديدة</span>
@endsection
...
<div dir="rtl" class="mx-auto w-full max-w-full space-y-6">
    ...
    <form id="budget-form" method="POST" action="{{ route('finance.budgets.store') }}" class="space-y-6" x-data='budgetCreateForm({
        accounts: @json($accountsTree),
        costCenters: @json($costCenters),
        initialItems: @json($defaultBudgetItems),
        monthKeys: @json($monthKeys),
        initialYear: @json((int) old('fiscal_year', $defaultYear)),
    })' x-init="setup()">
        @csrf
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div class="space-y-1">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="h-11 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div class="space-y-1">
                    <label class="inline-flex items-center gap-1 text-sm font-medium text-gray-700">
                    ...
                    <select name="fiscal_year" x-model.number="fiscalYear" @change="syncDates()" required class="h-11 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div class="space-y-1">
                    ...
                    <input type="date" name="end_date" x-model="endDate" required class="h-11 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div class="space-y-1">
                    <label class="inline-flex items-center gap-1 text-sm font-medium text-gray-700">
                        <span>الوصف</span>
                    ...
                    <textarea name="description" rows="3" class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description') }}</textarea>
                </div>
            </div>
        </section>
                ...
                <table class="w-full min-w-[1800px] text-sm">
                    <thead class="bg-gray-50 text-xs font-semibold text-gray-500">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right">حذف <x-info field="budget_actions" /></th>
                        </tr>
                    </thead>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" onclick="document.getElementById('budget-form').requestSubmit();" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">إنشاء</button>
        </div>
    </form>
</div>
@endsection
...
@push('scripts')
<script>
    function budgetCreateForm(config) {
        return {
