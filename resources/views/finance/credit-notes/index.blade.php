...
@section('title', 'إشعارات الائتمان - '.config('app.name'))
                ...
                <table class="w-full min-w-[980px] text-sm">
                    <thead class="bg-gray-50 text-xs font-semibold text-gray-500">
                        <tr>
                            ...
                            <th class="w-[1%] whitespace-nowrap px-4 py-3 text-center"><span class="inline-flex items-center justify-center gap-1"><x-info field="credit_note_actions" /> إجراءات</span></th>
                        </tr>
                    </thead>
                        ...
                        @forelse($creditNotes as $note)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center align-middle">
                                    ...
                                    <x-erp-actions-dropdown :menu-id="$cnMenuId">
                                        @if($note->status === 'draft')
                                            <button type="button"
                                                data-approve-url="{{ route('finance.credit-notes.approve', $note) }}"
                                                data-note-number="{{ $note->note_number }}"
                                                class="js-open-approve-modal erp-menu-item flex w-full items-center gap-3 px-3 py-2.5 text-right text-sm font-medium text-emerald-800 transition hover:bg-emerald-50"
                                                role="menuitem">
                                            ...
                                            <a href="{{ route('finance.credit-notes.edit', $note) }}"
                                            ...
                                            <div class="mx-2 my-2 border-t border-gray-100"></div>
                                            <form method="POST" action="{{ route('finance.credit-notes.destroy', $note) }}" class="m-0" onsubmit="return confirm('هل أنت متأكد من حذف هذا الإشعار؟ لا يمكن التراجع عن هذه الخطوة');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                        ...
                                        @elseif($note->status === 'approved')
                                            <a href="{{ route('finance.credit-notes.show', $note) }}"
                                               class="erp-menu-item flex items-center gap-3 px-3 py-2.5 text-sm text-gray-800 transition hover:bg-gray-50"
                                               role="menuitem">
                                                <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-600">
                                            ...
                                            <div class="mx-2 my-2 border-t border-gray-100"></div>
                                            <form method="POST" action="{{ route('finance.credit-notes.cancel', $note) }}" class="m-0" onsubmit="return confirm('هل تريد إلغاء الإشعار المعتمد؟ سيتم عكس القيد المحاسبي واسترجاع مديونية العميل.');">
                                                @csrf
                                                <button type="submit"
                                                        class="erp-menu-item flex w-full items-center gap-3 px-3 py-2.5 text-right text-sm font-medium text-red-700 transition hover:bg-red-50"
                                                        role="menuitem">
