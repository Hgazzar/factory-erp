...
@section('title', 'أعمار الذمم المدينة - '.config('app.name'))
...
@section('breadcrumb')
    ...
    <span class="mx-1 text-gray-400">›</span>
    <span class="text-blue-900 font-semibold">أعمار الذمم المدينة</span>
@endsection
...
<div dir="rtl" class="finance-aging-print-report mx-auto w-full max-w-full space-y-6">
    <section class="flex flex-wrap items-start justify-between gap-4 rounded-lg bg-white p-4 md:p-5">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">أعمار الذمم المدينة</h1>
            <p class="mt-1 text-sm text-gray-500">تحليل مستحقات العملاء حسب العمر</p>
        </div>
        <div class="no-print flex flex-wrap items-center gap-2">
            <a href="{{ route('finance.reports.ar-aging', ['as_of_date' => $asOfDate, 'export' => 'excel']) }}"
    ...
    <section class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <article class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <p class="mt-2 text-3xl font-bold text-gray-900">{{ (int) $stats['customers_count'] }}</p>
        </article>
    </section>

    <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        <div class="space-y-4 p-4">
            <form method="GET" action="{{ route('finance.reports.ar-aging') }}" class="no-print flex flex-wrap items-end justify-between gap-3">
                <div class="w-full max-w-xs space-y-1">
                    <label class="inline-flex items-center gap-1 text-xs font-medium text-gray-600">
                        <span>كما في</span>
                        <x-info field="ar_aging_as_of_date" />
                    </label>
            ...
            <div class="overflow-x-auto rounded-lg border border-gray-100">
                <table class="w-full min-w-[1080px] text-sm">
                    <thead class="bg-gray-50 text-xs font-semibold text-gray-500">
                        <tr>
                            <th class="px-4 py-3 text-right">الرمز <x-info field="ar_aging_customer_code" /></th>
                            <th class="px-4 py-3 text-right">اسم العميل <x-info field="ar_aging_customer_name" /></th>
                            <th class="px-4 py-3 text-right">حالي <x-info field="ar_aging_bucket_current" /></th>
                            <th class="px-4 py-3 text-right">1-30 يوم <x-info field="ar_aging_bucket_1_30" /></th>
                            <th class="px-4 py-3 text-right">31-60 يوم <x-info field="ar_aging_bucket_31_60" /></th>
                            <th class="px-4 py-3 text-right">61-90 يوم <x-info field="ar_aging_bucket_61_90" /></th>
                            <th class="px-4 py-3 text-right">أكثر من 90 يوم <x-info field="ar_aging_bucket_over_90" /></th>
                            <th class="px-4 py-3 text-right">الإجمالي <x-info field="ar_aging_total" /></th>
                        </tr>
                    </thead>
...
<style>
    @media print {
        ...
        /* عمود الاسم يسمح بالتفاف؛ باقي الأعمدة أرقام مضغوطة */
