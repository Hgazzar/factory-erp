...
@section('title', 'قسم جديد - '.config('app.name'))
...
<div class="max-w-full space-y-6" dir="rtl">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">قسم جديد</h1>
            <p class="mt-1 text-sm text-gray-500">إضافة قسم تنظيمي جديد</p>
        </div>
    ...
    <form method="POST" action="{{ route('hr.departments.store') }}" class="space-y-6">
        @csrf
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="name" class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-900 shadow-sm @error('name') border-red-500 ring-1 ring-red-200 @enderror" value="{{ old('name') }}" required maxlength="255" placeholder="مثال: الموارد البشرية">
                ...
                <div>
                    ...
                    <input type="text" name="name_en" class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-900 shadow-sm @error('name_en') border-red-500 ring-1 ring-red-200 @enderror" value="{{ old('name_en') }}" maxlength="255" placeholder="e.g. Human Resources">
                ...
                <div>
                    ...
                    <input type="text" name="code" class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 font-mono text-sm text-gray-900 shadow-sm @error('code') border-red-500 ring-1 ring-red-200 @enderror" value="{{ old('code') }}" maxlength="64" placeholder="HR, FIN, IT">
                ...
                <div
                    ...
                    x-data="{ active: @js((bool) old('is_active', true)) }"
                >
                    <span
                    ...
                    ></span>
                    <input type="hidden" name="is_active" value="{{ filter_var(old('is_active', true), FILTER_VALIDATE_BOOLEAN) ? '1' : '0' }}" x-bind:value="active ? 1 : 0">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-900 shadow-sm @error('description') border-red-500 ring-1 ring-red-200 @enderror" maxlength="10000" placeholder="أدخل وصف القسم">{{ old('description') }}</textarea>
                    @error('description')<p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
            </div>
        </section>

        {{-- التسلسل الهرمي: عمود واحد — القسم الأعلى ثم المدير --}}
        <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 md:items-start">
                <div>
                    ...
                    />
                    <p class="mt-1.5 text-xs text-gray-500">القسم الأعلى في التسلسل (اختياري)</p>
        ...
        <div class="flex flex-wrap items-center justify-end gap-3 border-t border-gray-100 pt-6">
            ...
            <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">
