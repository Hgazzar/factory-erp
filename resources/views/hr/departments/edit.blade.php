...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h4 mb-0">تعديل القسم</h1>
        ...
        <form method="POST" action="{{ route('hr.departments.update', $department) }}">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $department->name) }}" required maxlength="255">
                ...
                <div class="col-md-6">
                    ...
                    <select name="manager_id" class="form-select @error('manager_id') is-invalid @enderror">
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" @selected(old('manager_id', $department->manager_id) == $emp->id)>{{ $emp->name }} ({{ $emp->code }})</option>
                        @endforeach
                    </select>
            ...
            <hr class="my-4">
            <button type="submit" class="btn btn-primary rounded-lg px-5">تحديث</button>
        </form>
    </div>
</div>
@endsection
