...
@if($childSearchResults->isNotEmpty())
    <section class="nursery-card p-4">
        <h2 class="font-bold text-teal-950 mb-3">👶 أطفال — نتائج البحث</h2>
        <div class="space-y-2">
            @foreach($childSearchResults as $child)
                <div class="flex flex-wrap items-center justify-between gap-2 p-3 rounded-xl bg-teal-50 border border-teal-100">
                    <div>
                        <p class="font-semibold">{{ $child->name }}</p>
                        <p class="text-xs text-teal-800/70">{{ $child->code }} · {{ $child->guardian?->phone }}</p>
                    </div>
...
@if($staffSearchResults->isNotEmpty())
    <section class="nursery-card p-4">
        <h2 class="font-bold text-teal-950 mb-3">👥 طاقم العمل — نتائج البحث</h2>
        <div class="space-y-2">
            @foreach($staffSearchResults as $employee)
                <div class="flex flex-wrap items-center justify-between gap-2 p-3 rounded-xl bg-teal-50 border border-teal-100">
                    <div><p class="font-semibold">{{ $employee->name }}</p><p class="text-xs text-teal-800/70">{{ $employee->code }}</p></div>
                    <div class="flex gap-2">
                        @if($canManageStaff)
...
<section class="space-y-3">
    <div class="flex flex-wrap items-center gap-2">
        <h2 class="text-lg font-bold text-teal-950">👶 لوحة اليوم — {{ niche_label('entities.child', 'الأطفال') }}</h2>
        <x-info field="nursery.quick_check_in" />
        <span class="text-xs text-teal-700/70">({{ $board['date'] }})</span>
    </div>
...
@if($staffBoard !== null)
    <section class="space-y-3">
        <div class="flex flex-wrap items-center gap-2">
            <h2 class="text-lg font-bold text-teal-950">👥 لوحة اليوم — طاقم العمل</h2>
            <x-info field="nursery.staff_quick_attendance" />
            <span class="text-xs text-teal-700/70">({{ $staffBoard['date'] }})</span>
        </div>
        @include('nursery.partials.staff-today-board', ['staffBoard' => $staffBoard])
    </section>
@endif

<p class="text-xs text-teal-700/70">
