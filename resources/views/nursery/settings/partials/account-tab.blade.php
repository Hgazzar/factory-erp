...
<section class="nursery-card p-5 space-y-6" x-data="{ editing: @json($editing) }">
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-teal-100 pb-3">
        <h2 class="text-lg font-bold text-teal-950">المعلومات الأساسية</h2>
        @if($canManage)
    ...
    <div x-show="!editing" x-cloak class="space-y-6">
        <div>
            <h3 class="text-sm font-bold text-teal-900 mb-3">الهوية البصرية</h3>
            <div class="flex flex-wrap items-center gap-4 mb-2 p-3 rounded-lg bg-teal-50/50 border border-teal-100">
                <div class="w-14 h-14 rounded-xl border border-teal-200 bg-white flex items-center justify-center overflow-hidden p-1 shrink-0">
                    @if($settings->logoUrl())
                ...
                <div class="text-sm">
                    <p class="font-semibold text-teal-950">{{ $settings->portalDisplayName() }}</p>
                    @if($canManage)
                        <a href="{{ route('nursery.settings.index', ['tab' => 'branding']) }}" class="text-xs text-teal-600 font-bold hover:underline">رفع أو تغيير الشعار ←</a>
                    @endif
                </div>
            </div>
        </div>
        <div>
            <h3 class="text-sm font-bold text-teal-900 mb-3">بيانات الحضانة</h3>
        ...
        <div>
            <h3 class="text-sm font-bold text-teal-900 mb-3">بيانات مدير الحضانة</h3>
            <dl class="grid gap-3 sm:grid-cols-2 text-sm">
    ...
    @if($canManage)
        <form x-show="editing" x-cloak method="POST" action="{{ route('nursery.settings.account.update') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-semibold text-teal-950 mb-1">اسم الحضانة * <x-info field="nursery.settings_nursery_name" /></label>
                        <input type="text" name="nursery_name" required value="{{ old('nursery_name', $settings->nursery_name) }}" class="w-full rounded-lg border border-teal-200 px-3 py-2">
                        @error('nursery_name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-teal-950 mb-1">رقم التواصل <x-info field="nursery.settings_contact_phone" /></label>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="address" value="{{ old('address', $settings->address) }}" class="w-full rounded-lg border border-teal-200 px-3 py-2">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-teal-950 mb-1">المنطقة <x-info field="nursery.settings_region" /></label>
            ...
            <div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-semibold text-teal-950 mb-1">اسم المدير <x-info field="nursery.settings_manager_name" /></label>
                        <input type="text" name="manager_name" value="{{ old('manager_name', $settings->manager_name) }}" class="w-full rounded-lg border border-teal-200 px-3 py-2">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-teal-950 mb-1">رقم الجوال <x-info field="nursery.settings_manager_mobile" /></label>
