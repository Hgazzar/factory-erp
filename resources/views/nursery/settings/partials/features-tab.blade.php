<section class="nursery-card p-5 space-y-5">
    <div class="border-b border-teal-100 pb-3">
        <h2 class="text-lg font-bold text-teal-950">مزايا الحضانة <x-info field="nursery.settings_features_intro" /></h2>
        <p class="text-sm text-teal-800/80 mt-1">فعّل أو عطّل المزايا الإضافية لحسابك — تنعكس فوراً على البوابة والإشعارات والمالية.</p>
    </div>
        ...
        <form method="post" action="{{ route('nursery.settings.features.update') }}" class="space-y-4">
            ...
            <ul class="space-y-3">
                @foreach($featurePanel['features'] as $feature)
                    <li class="rounded-xl border border-teal-100 p-4 {{ $feature['locked'] ? 'bg-teal-50/40 opacity-80' : 'bg-white' }}">
                        <label class="flex items-start gap-3 {{ $feature['locked'] ? 'cursor-not-allowed' : 'cursor-pointer' }}">
                            ...
                            <span class="min-w-0 flex-1">
                                <span class="block font-semibold text-teal-950">
                                    {{ $feature['name_ar'] }}
                                    @if(!empty($feature['hint']))
                                        <x-info :field="'nursery.'.$feature['hint']" />
                                    @endif
                                </span>
                                <span class="block text-xs text-teal-800/75 mt-0.5">{{ $feature['description_ar'] }}</span>
    ...
    @else
        <ul class="space-y-3">
            @foreach($featurePanel['features'] as $feature)
                <li class="rounded-xl border border-teal-100 p-4 flex items-center justify-between gap-3">
                    <div>
                        <p class="font-semibold text-teal-950">{{ $feature['name_ar'] }}</p>
                        <p class="text-xs text-teal-800/75">{{ $feature['description_ar'] }}</p>
                    </div>
                    @if($feature['enabled'])
