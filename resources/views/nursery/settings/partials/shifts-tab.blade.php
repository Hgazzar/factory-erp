<section class="space-y-4" x-data="{ showAdd: {{ $errors->has('shifts') ? 'true' : 'false' }}, rows: [{ name: '', start_time: '', end_time: '' }] }">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <h2 class="text-lg font-bold text-teal-950">إدارة المناوبات <x-info field="nursery.settings_shifts_intro" /></h2>
        @if($canManage)
        ...
        <div class="overflow-x-auto">
            <table class="nursery-table min-w-[480px]">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $canManage ? 4 : 3 }}" class="!py-10 text-center text-teal-700/70">لا توجد مناوبات — أضف مناوبة عمل.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($canManage)
        <div x-show="showAdd" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" @keydown.escape.window="showAdd = false">
            <div class="nursery-card w-full max-w-lg max-h-[90vh] overflow-y-auto p-5 space-y-4" @click.outside="showAdd = false">
                <h3 class="text-lg font-bold text-teal-950">إضافة مناوبات</h3>
                <form method="POST" action="{{ route('nursery.settings.shifts.store') }}" class="space-y-4">
                    @csrf
                    <template x-for="(row, index) in rows" :key="index">
                        <div class="p-4 rounded-lg border border-teal-100 bg-teal-50/30 space-y-3">
                            <div>
                                <label class="block text-sm font-semibold text-teal-950 mb-1">اسم المناوبة * <x-info field="nursery.settings_shift_name" /></label>
                                <input type="text" :name="'shifts[' + index + '][name]'" x-model="row.name" required class="w-full rounded-lg border border-teal-200 px-3 py-2">
                            </div>
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-sm font-semibold text-teal-950 mb-1">وقت البداية * <x-info field="nursery.settings_shift_start" /></label>
                                    <input type="time" :name="'shifts[' + index + '][start_time]'" x-model="row.start_time" required class="w-full rounded-lg border border-teal-200 px-3 py-2">
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-teal-950 mb-1">وقت النهاية * <x-info field="nursery.settings_shift_end" /></label>
                                    <input type="time" :name="'shifts[' + index + '][end_time]'" x-model="row.end_time" required class="w-full rounded-lg border border-teal-200 px-3 py-2">
                                </div>
                            </div>
                        </div>
                    </template>
                    ...
                    <button type="button" @click="rows.push({ name: '', start_time: '', end_time: '' })"
                            class="text-sm font-semibold text-teal-600 hover:text-teal-800">+ إضافة حقل جديد</button>
