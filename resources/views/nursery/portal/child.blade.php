...
<div class="space-y-4">
    <a href="{{ route('nursery.portal.home', ['tenant_slug' => $tenantSlug]) }}"
       class="text-sm font-semibold text-orange-700 hover:text-orange-900">← أطفالي</a>

    <div class="np-card">
        <h2 class="text-xl font-extrabold text-orange-950">{{ $child->name }}</h2>
    ...
    <section class="np-card space-y-2">
        ...
        <p class="text-lg font-extrabold
            @class([
                ...
                'text-orange-700' => $todayStatus === 'no_record',
            ])">
            {{ $todayStatusLabel }}
        </p>
        @if($todayLog?->checked_in_at)
            <p class="text-sm text-orange-800/80">وقت الحضور: {{ $todayLog->checked_in_at->format('H:i') }}</p>
        @endif
        @if($todayLog?->checked_out_at)
            <p class="text-sm text-orange-800/80">وقت الانصراف: {{ $todayLog->checked_out_at->format('H:i') }}</p>
        @endif
    </section>
    ...
    <section class="np-card space-y-3">
        ...
        <dl class="space-y-2 text-sm">
            ...
            <div><dt class="text-orange-800/70 inline">الأمراض: </dt><dd class="inline font-medium">{{ $child->diseases ?: '—' }}</dd></div>
            <div><dt class="text-orange-800/70 inline">ملاحظات: </dt><dd class="inline font-medium">{{ $child->health_notes ?: '—' }}</dd></div>
        </dl>
    </section>
    ...
    <section class="np-card space-y-3">
        ...
        <p class="text-xs text-orange-700/70">عرض للاطلاع فقط — لا يمكن التعديل من البوابة.</p>
