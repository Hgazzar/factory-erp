...
<div class="w-full space-y-5" dir="rtl">
    @if($showTypePicker)
        <section class="nursery-card p-5 space-y-4">
            <h2 class="text-lg font-bold text-orange-950">ماذا تريد أن تضيف؟</h2>
            <div class="grid gap-3 sm:grid-cols-3">
                @foreach($typeLabels as $typeKey => $typeLabel)
                    <a href="{{ route('nursery.calendar.create', ['type' => $typeKey]) }}"
                       class="nursery-card p-4 text-center hover:ring-2 hover:ring-orange-400 transition {{ $entryType === $typeKey ? 'ring-2 ring-orange-500' : '' }}">
                        <p class="font-bold text-orange-950 text-lg">{{ $typeLabel }}</p>
                    </a>
                @endforeach
            </div>
        </section>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                        ...
                        @forelse($classrooms as $room)
                            <label class="nursery-age-row {{ $loop->iteration % 2 === 0 ? 'nursery-age-row--stripe' : '' }}">
                                <input type="checkbox" name="classroom_ids[]" value="{{ $room->id }}"
                                       class="rounded border-orange-300 text-orange-600 focus:ring-orange-500 shrink-0"
                                       @checked(in_array($room->id, $selectedClassroomIds, true))>
                                <span class="flex-1">{{ $room->name }}</span>
                            </label>
                        @empty
                            <p class="text-sm text-orange-800/70 p-3">لا توجد فصول نشطة.</p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="nursery-age-group-list max-h-[min(16rem,40vh)] overflow-y-auto">
                        ...
                        @empty
                            <p class="text-sm text-orange-800/70 p-3">لا يوجد أطفال نشطون.</p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea name="notes" rows="4" maxlength="5000" placeholder="اكتب شيئاً..."
                              class="w-full rounded-lg border border-orange-200 px-3 py-2 text-sm">{{ old('notes', $entry?->notes) }}</textarea>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm font-semibold text-orange-950">
                        تكرار
                ...
                @foreach($mediaLinks as $idx => $link)
                    <div class="calendar-media-row flex flex-wrap gap-2 items-center">
                        <input type="url" name="media_links[{{ $idx }}][url]" value="{{ $link['url'] ?? '' }}"
                               placeholder="https://..." class="flex-1 min-w-[12rem] rounded-lg border border-orange-200 px-3 py-2 text-sm" dir="ltr">
                        <input type="text" name="media_links[{{ $idx }}][label]" value="{{ $link['label'] ?? '' }}"
                               placeholder="وصف (اختياري)" class="w-full sm:w-40 rounded-lg border border-orange-200 px-3 py-2 text-sm">
                        <button type="button" class="calendar-remove-media nursery-btn nursery-btn-soft text-xs py-2 px-2">×</button>
                    </div>
                @endforeach
            </div>
            <p class="text-xs text-orange-700/70">قم بإضافة رابط (صورة أو ملف خارجي). رفع الملفات لاحقاً.</p>
        </section>
...
(function () {
    ...
    if (addMediaBtn && mediaList) {
        addMediaBtn.addEventListener('click', function () {
            ...
            row.innerHTML = '<input type="url" name="media_links[' + mediaIndex + '][url]" placeholder="https://..." class="flex-1 min-w-[12rem] rounded-lg border border-orange-200 px-3 py-2 text-sm" dir="ltr">'
                + '<input type="text" name="media_links[' + mediaIndex + '][label]" placeholder="وصف (اختياري)" class="w-full sm:w-40 rounded-lg border border-orange-200 px-3 py-2 text-sm">'
                + '<button type="button" class="calendar-remove-media nursery-btn nursery-btn-soft text-xs py-2 px-2">×</button>';
