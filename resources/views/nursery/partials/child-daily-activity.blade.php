...
<section class="nursery-card p-4 space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="font-bold text-teal-950">
                يوم الطفل
                <x-info field="nursery.child_daily_activity" />
            </h2>
            <p class="text-sm text-teal-800/80 mt-1">تسجيل سريع لما حدث اليوم — يظهر لولي الأمر ما هو مخصّص للبوابة فقط.</p>
        </div>
        <form method="get" action="{{ route('nursery.children.show', $child) }}" class="flex items-end gap-2">
            <div>
                <label class="block text-xs font-semibold text-teal-950 mb-1">التاريخ <x-info field="nursery.child_daily_activity_date" /></label>
                <input type="date" name="date" value="{{ $activityDate }}" max="{{ now()->toDateString() }}"
                       class="rounded-lg border border-teal-200 px-3 py-2 text-sm">
            </div>
    ...
    @if($dailySummary !== [])
        <div class="rounded-xl bg-teal-50 border border-teal-100 p-3 space-y-2 text-sm">
            @foreach($dailySummary as $group)
                <p>
                    <span class="font-bold text-teal-950">{{ $group['label'] }}:</span>
                    <span class="text-teal-800">{{ implode(' · ', $group['lines']) }}</span>
                </p>
            @endforeach
        </div>
    @endif
    ...
    @if($canManageChildActivity && $child->isActive())
        <div class="grid gap-3 sm:grid-cols-2">
            <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}" class="rounded-xl border border-teal-100 p-3 space-y-2">
                @csrf
                <input type="hidden" name="activity_date" value="{{ $activityDate }}">
                <input type="hidden" name="type" value="meal">
                <p class="text-sm font-bold text-teal-950">وجبة <x-info field="nursery.child_daily_meal" /></p>
            ...
            <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}" class="rounded-xl border border-teal-100 p-3 space-y-2">
                ...
                <p class="text-sm font-bold text-teal-950">قيلولة <x-info field="nursery.child_daily_nap" /></p>
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-xs text-teal-800/80 mb-1">من</label>
                        <input type="time" name="started_at" required class="w-full rounded-lg border border-teal-200 px-3 py-2 text-sm">
                    </div>
                    <div>
            ...
            <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}" class="rounded-xl border border-teal-100 p-3 space-y-2">
                ...
                <p class="text-sm font-bold text-teal-950">حفاض <x-info field="nursery.child_daily_diaper" /></p>
            ...
            <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}" class="rounded-xl border border-teal-100 p-3 space-y-2">
                ...
                <p class="text-sm font-bold text-teal-950">الحمام <x-info field="nursery.child_daily_toilet" /></p>
            ...
            <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}" class="rounded-xl border border-teal-100 p-3 space-y-2">
                ...
                <p class="text-sm font-bold text-teal-950">المزاج <x-info field="nursery.child_daily_mood" /></p>
            ...
            <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}" class="rounded-xl border border-teal-100 p-3 space-y-2">
                ...
                <p class="text-sm font-bold text-teal-950">نشاط <x-info field="nursery.child_daily_play" /></p>
            ...
            @if($child->relationLoaded('medications') ? $child->medications->isNotEmpty() : $child->medications()->exists())
                ...
                <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}"
                    ...
                    <p class="text-sm font-bold text-teal-950">جرعة دواء <x-info field="nursery.child_daily_medication" /></p>
                    <div>
                        <label class="block text-xs text-teal-800/80 mb-1">الدواء <x-info field="nursery.child_daily_medication_pick" /></label>
                    ...
                    <div>
                        <label class="block text-xs text-teal-800/80 mb-1">الحالة <x-info field="nursery.child_daily_medication_status" /></label>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label class="block text-xs text-teal-800/80 mb-1">الوقت <x-info field="nursery.child_daily_medication_time" /></label>
                            <input type="time" name="given_at" required class="w-full rounded-lg border border-teal-200 px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs text-teal-800/80 mb-1">الجرعة المعطاة <x-info field="nursery.child_daily_medication_dosage" /></label>
            ...
            <form method="post" action="{{ route('nursery.children.daily-activities.store', $child) }}" class="rounded-xl border border-teal-100 p-3 space-y-2 sm:col-span-2">
                @csrf
                <input type="hidden" name="activity_date" value="{{ $activityDate }}">
                <input type="hidden" name="type" value="note">
                <p class="text-sm font-bold text-teal-950">ملاحظة المعلمة <x-info field="nursery.child_daily_note" /></p>
                <textarea name="note" rows="2" maxlength="500" required placeholder="ملاحظة داخلية…"
                          class="w-full rounded-lg border border-teal-200 px-3 py-2 text-sm"></textarea>
                <label class="flex items-center gap-2 text-sm text-teal-900">
                    <input type="checkbox" name="is_parent_visible" value="1" class="rounded border-teal-300 text-teal-600">
                    إظهار لولي الأمر
    ...
    @elseif(! $child->isActive())
        <p class="text-sm text-teal-800/70">الطفل مؤرشف — لا يمكن تسجيل يوم جديد.</p>
    @endif

    <div>
        <h3 class="text-sm font-bold text-teal-950 mb-2">سجل {{ $activityDate }}</h3>
        <ul class="space-y-2">
            @forelse($todaysActivities as $item)
                <li class="flex flex-wrap items-start justify-between gap-2 rounded-lg border border-teal-50 bg-white px-3 py-2 text-sm">
                    <div class="min-w-0">
                        <p class="font-semibold text-teal-950">{{ $item->typeLabel() }}</p>
                        <p class="text-teal-800/90">{{ $item->summaryLine() }}</p>
                        <p class="text-xs text-teal-700/70 mt-0.5">
            ...
            @empty
                <li class="text-sm text-teal-700/60">لا يوجد سجل لهذا اليوم.</li>
            @endforelse
        </ul>
    </div>
</section>
