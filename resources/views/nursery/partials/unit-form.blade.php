...
<form method="post" action="{{ $formAction }}" class="space-y-5" id="nurseryUnitForm">
    ...
    <div class="grid gap-5 lg:grid-cols-2">
        <section class="nursery-card p-5 space-y-4 lg:col-span-2">
            <h2 class="text-lg font-bold text-teal-950 border-b border-teal-100 pb-2">بيانات الوحدة</h2>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="nursery-age-group-list max-h-[min(22rem,55vh)] overflow-y-auto">
                        <label class="nursery-age-row nursery-age-row--header">
                            <input type="checkbox" id="nursery_unit_age_groups_select_all" class="rounded border-teal-300 text-teal-600 focus:ring-teal-500 shrink-0">
                        ...
                        @foreach($ageGroupLabels as $key => $label)
                            <label class="nursery-age-row {{ $loop->iteration % 2 === 0 ? 'nursery-age-row--stripe' : '' }}">
                                <input type="checkbox" name="age_groups[]" value="{{ $key }}"
                                       class="nursery-unit-age-group-cb rounded border-teal-300 text-teal-600 focus:ring-teal-500 shrink-0"
                ...
                <div class="lg:col-span-2">
                    <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
                        <label class="text-sm font-semibold text-teal-950">
                    ...
                    <div id="nursery_unit_goals_list" class="space-y-2">
                        @foreach($goalLines as $index => $goalText)
                            <div class="nursery-unit-goal-row flex gap-2 items-start">
                                <span class="text-sm text-teal-800/70 pt-2 tabular-nums w-6 shrink-0">{{ $loop->iteration }}.</span>
                                <input type="text" name="goals[]" value="{{ $goalText }}"
                                       class="flex-1 rounded-lg border border-teal-200 px-3 py-2 text-sm"
    ...
    <div class="flex flex-wrap gap-2">
        ...
        @if($unit === null)
            <button type="submit" name="submit_action" value="save_and_details" class="nursery-btn nursery-btn-soft border-teal-300">حفظ وإكمال التفاصيل</button>
...
(function () {
    ...
    if (addBtn && goalsList) {
        addBtn.addEventListener('click', function () {
            ...
            row.innerHTML = '<span class="text-sm text-teal-800/70 pt-2 tabular-nums w-6 shrink-0"></span>'
                + '<input type="text" name="goals[]" class="flex-1 rounded-lg border border-teal-200 px-3 py-2 text-sm" placeholder="هدف جديد">'
                + '<button type="button" class="nursery-unit-remove-goal nursery-btn nursery-btn-soft text-xs py-2 px-2 shrink-0" title="حذف">×</button>';
    ...
    if (addLessonBtn && lessonsList) {
        addLessonBtn.addEventListener('click', function () {
            ...
            row.innerHTML = '<input type="text" name="lessons[]" class="flex-1 rounded-lg border border-teal-200 px-3 py-2 text-sm" placeholder="اسم الدرس">'
                + '<button type="button" class="nursery-unit-remove-lesson nursery-btn nursery-btn-soft text-xs py-2 px-2 shrink-0" title="حذف">×</button>';
