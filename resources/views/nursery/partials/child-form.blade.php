...
<form method="post" action="{{ $formAction }}" enctype="multipart/form-data" class="space-y-5" id="nurseryChildForm">
    ...
    <div class="grid gap-5 lg:grid-cols-2 items-start">
        <section class="nursery-card p-5 space-y-4 lg:col-span-2">
            <h2 class="text-lg font-bold text-teal-950 border-b border-teal-100 pb-2">المعلومات الأساسية</h2>
        ...
        <section class="nursery-card p-5 space-y-4">
            <h2 class="text-lg font-bold text-teal-950 border-b border-teal-100 pb-2">المعلومات الصحية</h2>
            <div>
                <label class="block text-sm font-semibold text-teal-950 mb-1">
                    الحساسية
                    <x-info field="nursery.allergies" />
                </label>
                <textarea name="allergies" rows="2" class="w-full rounded-lg border border-teal-200 px-3 py-2">{{ old('allergies', $child?->allergies) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-semibold text-teal-950 mb-1">
                    الأمراض
                ...
                <textarea name="diseases" rows="2" class="w-full rounded-lg border border-teal-200 px-3 py-2">{{ old('diseases', $child?->diseases) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-semibold text-teal-950 mb-1">
                    ملاحظات صحية
                ...
                <textarea name="health_notes" rows="2" class="w-full rounded-lg border border-teal-200 px-3 py-2">{{ old('health_notes', $child?->health_notes) }}</textarea>
            </div>
            @include('nursery.partials.child-medications', ['child' => $child ?? null])
        </section>

        <section class="nursery-card p-5 space-y-4">
            <h2 class="text-lg font-bold text-teal-950 border-b border-teal-100 pb-2">معلومات ولي الأمر</h2>
            <div class="grid gap-4 lg:grid-cols-2">
                <div class="lg:col-span-2">
                    <label class="block text-sm font-semibold text-teal-950 mb-1">
                        الاسم <span class="text-red-600">*</span>
    ...
    <div class="flex flex-wrap gap-2 items-center">
        ...
        @canFeature(\App\Support\PremiumFeatureKeys::NURSERY_PORTAL)
            <label class="inline-flex items-center gap-2 text-sm text-teal-900 cursor-pointer">
                <input type="checkbox" name="send_portal_invite" value="1" class="rounded border-teal-300 text-teal-600">
