...
<div x-show="careOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40"
    ...
    <div class="nursery-card w-full max-w-md p-5 space-y-4 max-h-[90vh] overflow-y-auto" @click.outside="closeCare()">
        <div class="flex items-start justify-between gap-2">
            <div>
                <h3 class="text-lg font-bold text-orange-950">تسجيل نشاط</h3>
                <p class="text-sm text-orange-800/80" x-text="careChildName"></p>
            </div>
        ...
        <form method="post" class="space-y-3" :action="'{{ url('/nursery/children') }}/' + careChildId + '/daily-activities'">
            ...
            <div x-show="careType === 'nap'" class="grid grid-cols-2 gap-2">
                <div>
                    <label class="block text-xs text-orange-800/80 mb-1">من</label>
                        <input type="time" name="started_at" class="w-full rounded-lg border border-orange-200 px-3 py-2 text-sm" :required="careType === 'nap'">
                </div>
                <div>
            ...
            <div x-show="careType === 'medication'" class="space-y-2">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-xs text-orange-800/80 mb-1">الوقت</label>
                        <input type="time" name="given_at" class="w-full rounded-lg border border-orange-200 px-3 py-2 text-sm"
                               :required="careType === 'medication'">
                    </div>
                    <div>
                        <label class="block text-xs text-orange-800/80 mb-1">الجرعة</label>
