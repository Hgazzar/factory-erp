...
@section('title', 'تعديل عرض السعر - MIRADA ERP')
...
@section('breadcrumb')
    ...
    <span>›</span>
    <span class="text-indigo-900 font-semibold">تعديل عرض السعر</span>
@endsection

@section('content')
<div class="max-w-full" x-data="quotationCreateForm(@js($items), @js($initialLines), @js(route('api.products.search')))" x-cloak>
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-2xl font-bold text-gray-900">تعديل عرض السعر</h1>
        </div>
    </div>

    <form id="quotation-form" method="POST" action="{{ route('sales.quotations.update', $quotation) }}" class="space-y-6" @submit="if (!guardLineItems($event)) return">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="customer_id" required class="w-full px-3 py-2.5 pr-4 text-right bg-gray-50 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('customer_id') border-red-500 @enderror">
                        ...
                        @foreach($customers as $c)
                            <option value="{{ $c->id }}" @selected(old('customer_id', $quotation->customer_id) == $c->id)>{{ $c->display_name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <label for="quotation-date" class="relative flex cursor-pointer">
                        <input type="date" id="quotation-date" name="date" required value="{{ old('date', $quotation->date?->format('Y-m-d')) }}" class="w-full py-2.5 pl-10 pr-4 text-right bg-gray-50 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div>
                    ...
                    <label for="quotation-valid-until" class="relative flex cursor-pointer">
                        <input type="date" id="quotation-valid-until" name="valid_until" required value="{{ old('valid_until', $quotation->valid_until?->format('Y-m-d')) }}" class="w-full py-2.5 pl-10 pr-4 text-right bg-gray-50 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-400 pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16"><path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5zM1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4H1z"/><path d="M2.5 3V2h11v1h-11z"/></svg>
                        </span>
                    </label>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <textarea name="notes" rows="3" class="w-full px-3 py-2 pr-4 text-right bg-gray-50 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="ملاحظات للعميل">{{ old('notes', $quotation->notes) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">ملاحظات داخلية</label>
                    <textarea name="internal_notes" rows="3" class="w-full px-3 py-2 pr-4 text-right bg-gray-50 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="للاستخدام الداخلي فقط">{{ old('internal_notes', $quotation->internal_notes) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">الشروط والأحكام</label>
                    <textarea name="terms" rows="3" class="w-full px-3 py-2 pr-4 text-right bg-gray-50 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="الشروط والأحكام">{{ old('terms', $quotation->terms) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-5 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 transition">حفظ التعديلات</button>
        </div>
    </form>
</div>

<script>
window.quotationCreateForm = function(items, initialLines, searchUrl) {
            ...
            if (Array.isArray(initialLines) && initialLines.length > 0) {
                lines = initialLines.map(function (l) {
                    return Object.assign(emptyLine(), {
                        item_id: l.item_id != null ? String(l.item_id) : '',
                        item_search: findItemDisplay(l.item_id),
            ...
            return {
                ...
                guardLineItems(e) {
                    if (this.lines.some((l) => !String(l.item_id || '').trim())) {
                        e.preventDefault();
                        alert('يرجى اختيار منتجاً من نتائج البحث لكل بند (اضغط على صف في القائمة بعد البحث).');
                        return false;
                    }
                    return true;
                },
                ...
                selectItem(lineIndex, item) {
                    const unitPrice = parseFloat(item.sale_price ?? item.selling_price ?? 0) || 0;
                    ...
                    this.lines[lineIndex].unit_price = unitPrice;
