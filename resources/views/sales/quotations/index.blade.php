...
<div class="max-w-full">
    ...
    <form method="GET" action="{{ route('sales.quotations.index') }}" class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-6">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <select name="customer_id" class="py-2 px-3 border border-gray-300 rounded-lg text-sm bg-white min-w-[160px] text-right">
                <option value="">جميع العملاء</option>
                @foreach($customers as $c)
                    <option value="{{ $c->id }}" {{ request('customer_id') == $c->id ? 'selected' : '' }}>{{ $c->display_name }}</option>
                @endforeach
            </select>
...
<style>
    /* منع أي سكرول داخل القائمة بسبب أنماط عامة */
    .erp-actions-menu {
...
<script>
    (function () {
        function positionMenu(trigger, menu) {
            ...
            // فتح القائمة بمحاذاة حافتها اليسرى بعد يمين الزر (نحو داخل الصفحة) لتجنب القص على حافة الشاشة
        ...
        function restoreMenuToCell(menu) {
            if (!menu._erpPlaceholder || !menu._erpPlaceholder.parentNode) {
                return;
            }
