...
@section('content')
@php
    $invoiceCustomerOptions = $customers->map(fn ($c) => [
        'value' => $c->id,
        'label' => (string) ($c->display_name ?? $c->name ?? ''),
    ])->all();
@endphp
<div class="max-w-full" x-data="invoiceCreateForm(@js($items), @js($initialLines ?? []), @js($defaultVatPercent))" x-cloak>
...
window.invoiceCreateForm = function(items, initialLines, defaultVatPercent) {
        ...
        const linesFromQuotation = Array.isArray(initialLines) && initialLines.length > 0
            ? initialLines.map((l) => ({
                ...
                tax_percent: l.tax_percent != null && l.tax_percent !== '' ? parseFloat(l.tax_percent) : vatDef,
            }))
            : null;
        return {
            submitting: false,
